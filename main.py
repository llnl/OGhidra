...
def run_interactive_mode(bridge: Bridge, config: BridgeConfig):
    ...
    while True:
        try:
            user_input = input(
                "Query (or 'exit', 'help', 'health', 'analyze-function', 'enumerate-binary', 'review-session'): "
            )
            if not user_input:
                continue
                ...
                if bridge.enable_cag and bridge.cag_manager:
                    ...
                    try:
                        vector_store_enabled = (
                            config.session_history.use_vector_embeddings if hasattr(config, "session_history") else False
                        )
                        ...
                        if vector_store_enabled and hasattr(bridge, "memory_manager") and bridge.memory_manager is not None:
                            ...
                            if mm.vector_store:
                                vector_count = (
                                    mm.vector_store.vectors.shape[0]
                                    if (hasattr(mm.vector_store, "vectors") and mm.vector_store.vectors is not None)
                                    else 0
                                )
                        ...
                        for pair in param_pairs:
                            if "=" in pair:
                                ...
                                if (value_str_from_pair.startswith("'") and value_str_from_pair.endswith("'")) or (
                                    ...
                                    final_value_for_param = (
                                        value_str_from_pair[1:-1].encode("utf-8").decode("unicode_escape")
                                    )  # Handle escapes
                                else:  # Try to infer type for unquoted values
                                    if value_str_from_pair.lower() == "true":
                                        final_value_for_param = True
                                    elif value_str_from_pair.lower() == "false":
                                        final_value_for_param = False
                                    elif value_str_from_pair.lower() == "none":
                                        ...
                                        value_str_from_pair.startswith("-") and value_str_from_pair[1:].isdigit()
                                    ):  # Negative integers
                                        final_value_for_param = int(value_str_from_pair)
                        ...
                        if tool_name in TOOLS_WITH_AI_ANALYSIS:
                            ...
                            if not is_error:
                                ...
                                current_session_log.append(
                                    f"=== Raw Output from {tool_name}({params_for_log}) ===\\n{formatted_tool_data}\\n"
                                )
...
```"""
                                ...
                                if analysis_prompt:
                                    ...
                                    try:
                                        ...
                                        print(
                                            f"DEBUG: AI Response Type: {type(ai_analysis)}, Is None: {ai_analysis is None}, Is Empty Str: {ai_analysis == ''}, Length: {len(ai_analysis) if ai_analysis else 0}"
                                        )

                                        if ai_analysis == "Ollama client not available.":
                                            print(f"\n{ai_analysis}")
                                            current_session_log.append(
                                                f"=== AI Analysis of {tool_name}({params_for_log}): Ollama client not available ===\\n"
                                            )
                                            ...
                                            print("=====================================")
                                            current_session_log.append(
                                                f"=== AI Analysis of {tool_name}({params_for_log}) ===\\n{ai_analysis}\\n"
                                            )
                                        else:
                                            print("\nAI analysis returned empty or whitespace-only response.")
                                            current_session_log.append(
                                                f"=== AI Analysis of {tool_name}({params_for_log}) returned empty. ===\\n"
                                            )
                                    ...
                                    print(
                                        f"No specific AI analysis prompt configured for tool: {tool_name}. Raw output printed above."
                                    )
                            else:  # Error in raw_tool_result
                                print(f"Error from tool {tool_name}: {raw_tool_result}")
                                current_session_log.append(
                                    f"=== Error from tool {tool_name}({params_for_log}): {raw_tool_result} ===\\n"
                                )
                            ...
                            current_session_log.append(
                                f"=== Result of {tool_name}({params_for_log}) ===\\n{raw_tool_result}\\n"
                            )
                    else:
                ...
                try:
                    ...
                    if raw_tool_result != "Ghidra client not available." and not (
                        ...
                        try:
                            ...
                            if ai_analysis == "Ollama client not available.":
                                print(f"\n{ai_analysis}")
                                current_session_log.append(
                                    f"=== AI Analysis of analyze-function({params_for_log}): Ollama client not available ===\\n"
                                )
                            elif ai_analysis and ai_analysis.strip():
                                print("\n=== AI Analysis of Function Output ===")
                                print(ai_analysis)
                                print("=====================================")
                                current_session_log.append(
                                    f"=== AI Analysis of analyze-function({params_for_log}) ===\\n{ai_analysis}\\n"
                                )
                            else:
                                print("\nAI analysis returned empty or whitespace-only response.")
                                current_session_log.append(
                                    f"=== AI Analysis of analyze-function({params_for_log}) returned empty. ===\\n"
                                )
...
CRITICAL: You MUST include all four sections with the exact headers shown above. Focus on making the suggested name as specific and descriptive as possible."""
                    ...
                    if not processed_functions_data:
                        ...
                        if not (
                            hasattr(bridge, "enable_cag")
                            and bridge.enable_cag
                            and hasattr(bridge, "cag_manager")
                            and bridge.cag_manager
                        ):
                ...
                review_query = input(
                    "What would you like to ask about the work done in this session? (Type 'cancel' to abort): "
                )
                ...
                try:
                    ...
                    if (
                        ai_review_response
                        and ai_review_response.strip()
                        and ai_review_response != "Ollama client not available."
                    ):
