...
class TestCommandNormalization(unittest.TestCase):
    ...
    def test_bridge_normalize_command_name(self):
        ...
        self.assertEqual(self.bridge._normalize_command_name("getCurrentFunction"), "get_current_function")

        # 2. Already snake_case name remains unchanged
        self.assertEqual(self.bridge._normalize_command_name("get_current_function"), "get_current_function")

        # 3. camelCase that exists on the object remains unchanged
        self.assertEqual(self.bridge._normalize_command_name("camelCaseMethod"), "camelCaseMethod")

        # 4. Unknown commands in any case format remain unchanged
        self.assertEqual(self.bridge._normalize_command_name("nonExistentCommand"), "nonExistentCommand")
