...
class FunctionGraph:
    ...
    def expand_context_for_rag(self, primary_results: list[str], expansion_depth: int = 1, max_expanded: int = 10) -> list[str]:
        ...
        for address in primary_results:
            ...
            related = self.get_related_functions(address, depth=expansion_depth, include_callers=True, include_callees=True)
    ...
    def get_statistics(self) -> dict[str, Any]:
        ...
        # Find isolated nodes
        isolated = sum(1 for node in self.nodes.values() if len(node.callers) == 0 and len(node.callees) == 0)
