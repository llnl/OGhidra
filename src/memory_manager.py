...
class MemoryManager:
    ...
    def __init__(self, config: BridgeConfig, llm_client=None):
        ...
        self.session_store = SessionHistoryStore(storage_path=self.session_config.storage_path)
        ...
        if self.session_config.use_vector_embeddings:
            self.vector_store = SimpleVectorStore(storage_dir=self.session_config.vector_db_path)

        # Initialize embedder and summarizer – use embedding model, not generation model
        self.embedder = SessionEmbedder(embedding_model=config.ollama.embedding_model)
    ...
    def log_tool_call(
        ...
        result = self.active_session_manager.log_tool_call(
            tool_name=tool_name, parameters=parameters, status=status, result_preview=result_preview
        )
        if result:
    ...
    def end_session(self, outcome: str, reason: str | None = None, generate_summary: bool = None) -> str | None:
        ...
        # End the session
        result = self.active_session_manager.end_current_session(outcome=outcome, reason=reason, summary=summary)
    ...
    def get_similar_sessions(self, query: str, top_k: int = 3) -> list[dict[str, Any]]:
        ...
        dummy_session = SessionRecord(user_task_description=query, outcome="in_progress")
    ...
    def clear_all_sessions(self) -> bool:
        ...
        try:
            ...
            if self.active_session_manager.current_session:
                self.active_session_manager.end_current_session(outcome="aborted", reason="Cleared all sessions")
