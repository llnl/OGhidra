...
class ExecutionGatekeeper:
    ...
    CRITICAL_ARTIFACT_PATTERNS = [
        ...
        (
            r"(?i)CryptEncrypt|CryptDecrypt|BCryptEncrypt|BCryptDecrypt",
            "Cryptographic operation detected",
        ),
        (
            r"(?i)(?:password|passwd|credential|secret)\s*[:=]",
        ...
        (
            ...
            "Hardcoded IP URL (possible C2)",
        ),
        # Shellcode / injection patterns
        (
            r"VirtualAlloc.*PAGE_EXECUTE",
            "Executable memory allocation (possible shellcode)",
        ),
    ...
    def __init__(self, config):
        ...
        self._compiled_patterns = [
            (re.compile(pattern), description) for pattern, description in self.CRITICAL_ARTIFACT_PATTERNS
        ]
    ...
    def _extract_artifacts_from_findings(
        ...
        for matched in matched_artifacts:
            ...
            artifacts.append({"category": category, "key": key, "value": value})
