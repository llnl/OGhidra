...
class GhidraMCPClient:
    ...
    def rename_function(self, old_name: str, new_name: str) -> str:
        ...
        """
        try:
            response = self._post_request("rename", {"old_name": old_name, "new_name": new_name})
    ...
    def rename_function_by_address(self, function_address: str, new_name: str) -> str:
        ...
        """
        try:
            response = self._post_request("rename", {"function_address": function_address, "new_name": new_name})
    ...
    def list_imports(self, offset: int = 0, limit: int = 100) -> List[str]:
        ...
        """
        try:
            response = self._get_request("imports", {"offset": offset, "limit": limit})
    ...
    def list_segments(self, offset: int = 0, limit: int = 100) -> List[str]:
        ...
        """
        try:
            response = self._get_request("segments", {"offset": offset, "limit": limit})
            # Parse the response - assuming it's JSON
            try:
                return response.json()
            except json.JSONDecodeError as e:
    ...
    def search_functions_by_name(self, query: str, offset: int = 0, limit: int = 100) -> List[str]:
        ...
        """
        try:
            response = self._get_request("functions/search", {"query": query, "offset": offset, "limit": limit})
            # Parse the response - assuming it's JSON
            try:
                return response.json()
            except json.JSONDecodeError as e:
