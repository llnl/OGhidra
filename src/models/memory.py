...
class PromptSection(BaseModel):
    ...
    model_config = ConfigDict(frozen=True)  # Make immutable for consistent ordering
...
class FunctionRegistry(BaseModel):
    ...
    def register(self, analysis: FunctionAnalysis):
        ...
        with self._lock:
            ...
            if auto_tag in existing.purpose and auto_tag not in analysis.purpose and analysis.purpose:
                existing.purpose = analysis.purpose
            elif len(analysis.purpose) > len(existing.purpose) and auto_tag not in analysis.purpose:
                existing.purpose = analysis.purpose
            ...
            if confidence_order.get(analysis.confidence, 0) > confidence_order.get(existing.confidence, 0):
            ...
            if analysis.name and not analysis.name.startswith("FUN_") and existing.name.startswith("FUN_"):
    ...
    def format_for_prompt(self, max_entries: int = 20) -> str:
        ...
        with self._lock:
            ...
            sorted_funcs = sorted(self.functions.values(), key=lambda f: (bool(f.security_notes), f.timestamp), reverse=True)
    ...
    def format_for_orchestrator(self, max_entries: int = 30) -> str:
        ...
        with self._lock:
            ...
            lines = [f"## Function Registry " f"({len(self.functions)} registered, {self.analyzed_count} analyzed)"]

            if over_decompiled:
                lines.append("\n### OVER-ANALYZED (DO NOT re-decompile):")
                for f in over_decompiled[:10]:
                    lines.append(f"- {f.name} ({f.address}) \u2014 decompiled " f"{f.decompile_count}x. {f.purpose[:60]}")
            ...
            if analyzed:
                ...
                for f in sorted_analyzed[:max_entries]:
                    sec = f" \u26a0 {', '.join(f.security_notes[:2])}" if f.security_notes else ""
                    lines.append(f"- {f.name} ({f.address}): {f.purpose[:80]}{sec}")

            if shallow:
                lines.append(f"\n### Seen But Not Analyzed " f"({len(shallow)} \u2014 consider investigating):")
                for f in shallow[:10]:
...
class DiscoveryCache:
    ...
    def is_empty(self) -> bool:
        with self._lock:
            return not self.imports and not self.exports and not self.strings and not self.functions_list

    # ── Prompt formatting ──────────────────────────────────────────

    def format_for_prompt(self, max_imports: int = 40, max_strings: int = 20) -> str:
        ...
        with self._lock:
            ...
            lines = ["## Binary Discovery (cached — do NOT re-call these tools)"]
            ...
            if self.imports:
                ...
                if len(self.imports) > max_imports:
                    lines.append(f"  ... and {len(self.imports) - max_imports} more imports")
            ...
            if self.exports:
                ...
                if len(self.exports) > 20:
                    lines.append(f"  ... and {len(self.exports) - 20} more exports")
            ...
            if self.strings:
                ...
                for filt, entries in self.strings.items():
                    ...
                    lines.append(f'Filter "{filt}": {shown}')
                    if len(entries) > max_strings:
                        lines.append(f"  ... and {len(entries) - max_strings} more")

            # Functions
            if self.functions_list:
                total_label = f" of {self.total_functions}" if self.total_functions > 0 else ""
                lines.append(f"### Functions ({len(self.functions_list)}{total_label} listed)")
                lines.append(", ".join(self.functions_list[:30]))
                if len(self.functions_list) > 30:
                    lines.append(f"  ... and {len(self.functions_list) - 30} more")
