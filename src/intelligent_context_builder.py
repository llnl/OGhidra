...
class IntelligentContextBuilder:
    ...
    def build_context_for_query(self, function_addr: str, query: str, max_tokens: int = 2000) -> str:
        """
    ...
    def _build_security_context(self, func_data: Dict, max_tokens: int) -> str:
        ...
        if operations:
            security_ops = [
                op for op in operations if any(s in op for s in ["crypto", "network", "auth", "registry", "process"])
            ]
            if security_ops:
