...
class ServerConfigDialog:
    ...
    def _setup_widgets(self):
        ...
        title_label = ttk.Label(
            main_frame, text="Server Configuration", font=("TkDefaultFont", 12, "bold")
        )
        ...
        self.external_frame = ttk.LabelFrame(
            main_frame, text="External Provider Configuration", padding=10
        )

        # Provider Type
        ttk.Label(self.external_frame, text="Provider Type:").grid(
            row=0, column=0, sticky="w", pady=5
        )
        self.ext_provider_var = tk.StringVar(
            value=getattr(self.config.external, "provider", "google")
        )
        ...
        ttk.Label(self.external_frame, text="API Key:").grid(
            row=1, column=0, sticky="w", pady=5
        )
        self.ext_key_var = tk.StringVar(
            value=getattr(self.config.external, "api_key", "")
        )
        ext_key_entry = ttk.Entry(
            self.external_frame, textvariable=self.ext_key_var, width=50, show="*"
        )
        ext_key_entry.grid(row=1, column=1, sticky="ew", padx=(10, 0), pady=5)

        ttk.Label(self.external_frame, text="Model:").grid(
            row=2, column=0, sticky="w", pady=5
        )
        self.ext_model_var = tk.StringVar(
            value=getattr(self.config.external, "model", "gemini-1.5-flash")
        )
        ext_model_entry = ttk.Entry(
            self.external_frame, textvariable=self.ext_model_var, width=50
        )
        ext_model_entry.grid(row=2, column=1, sticky="ew", padx=(10, 0), pady=5)

        ttk.Label(self.external_frame, text="Embedding Model:").grid(
            row=3, column=0, sticky="w", pady=5
        )
        self.ext_embed_var = tk.StringVar(
            value=getattr(self.config.external, "embedding_model", "text-embedding-004")
        )
        ext_embed_entry = ttk.Entry(
            self.external_frame, textvariable=self.ext_embed_var, width=50
        )
        ...
        self.custom_api_frame = ttk.LabelFrame(
            main_frame, text="Custom API Configuration (OpenAI-compatible)", padding=10
        )

        ttk.Label(self.custom_api_frame, text="API URL:").grid(
            row=0, column=0, sticky="w", pady=5
        )
        ...
        custom_url_entry = ttk.Entry(
            self.custom_api_frame, textvariable=self.custom_api_url_var, width=50
        )
        custom_url_entry.grid(row=0, column=1, sticky="ew", padx=(10, 0), pady=5)

        ttk.Label(self.custom_api_frame, text="API Key:").grid(
            row=1, column=0, sticky="w", pady=5
        )
        self.custom_api_key_var = tk.StringVar(
            value=getattr(self.config.custom_api, "api_key", "")
        )
        ...
        ttk.Label(self.custom_api_frame, text="Model:").grid(
            row=2, column=0, sticky="w", pady=5
        )
        self.custom_api_model_var = tk.StringVar(
            value=getattr(self.config.custom_api, "model", "gpt-4")
        )
        custom_model_entry = ttk.Entry(
            self.custom_api_frame, textvariable=self.custom_api_model_var, width=50
        )
        custom_model_entry.grid(row=2, column=1, sticky="ew", padx=(10, 0), pady=5)

        ttk.Label(self.custom_api_frame, text="Embedding Model:").grid(
            row=3, column=0, sticky="w", pady=5
        )
        self.custom_api_embed_var = tk.StringVar(
            value=getattr(
                self.config.custom_api, "embedding_model", "text-embedding-ada-002"
            )
        )
        custom_embed_entry = ttk.Entry(
            self.custom_api_frame, textvariable=self.custom_api_embed_var, width=50
        )
        custom_embed_entry.grid(row=3, column=1, sticky="ew", padx=(10, 0), pady=5)

        ttk.Label(self.custom_api_frame, text="Max Tokens:").grid(
            row=4, column=0, sticky="w", pady=5
        )
        self.custom_api_max_tokens_var = tk.StringVar(
            value=str(getattr(self.config.custom_api, "max_tokens", 4096))
        )
        custom_tokens_entry = ttk.Entry(
            self.custom_api_frame, textvariable=self.custom_api_max_tokens_var, width=50
        )
        ...
        ollama_entry = ttk.Entry(
            self.ollama_frame, textvariable=self.ollama_url_var, width=50
        )
        ...
        model_entry = ttk.Entry(
            self.ollama_frame, textvariable=self.ollama_model_var, width=50
        )
        model_entry.grid(row=1, column=1, sticky="ew", padx=(10, 0), pady=5)

        # Embedding model selection
        ttk.Label(self.ollama_frame, text="Embedding Model:").grid(
            row=2, column=0, sticky="w", pady=5
        )
        self.embedding_model_var = tk.StringVar(
            value=getattr(self.config.ollama, "embedding_model", "nomic-embed-text")
        )
        embedding_entry = ttk.Entry(
            self.ollama_frame, textvariable=self.embedding_model_var, width=50
        )
        ...
        ttk.Label(ghidra_frame, text="Base URL:").grid(
            row=0, column=0, sticky="w", pady=5
        )
        self.ghidra_url_var = tk.StringVar(value=str(self.config.ghidra.base_url))
        ghidra_entry = ttk.Entry(
            ghidra_frame, textvariable=self.ghidra_url_var, width=50
        )
        ...
        test_button = ttk.Button(
            button_frame, text="Test Connections", command=self._test_connections
        )
            ...
            if provider == "google" or provider == "external":
                    ...
                    if not api_key:
                        ...
                        if response.status_code == 200:
                            ...
                            if model_found:
                                ...
                                results.append(
                                    f"Model ({target_model}): ⚠️ Not found in list (might still work)"
                                )
                        else:
                            results.append(
                                f"External API: ❌ HTTP {response.status_code}"
                            )
                ...
                try:
                    ...
                    if not api_key:
                        ...
                        if response.status_code == 200:
                            ...
                            results.append(
                                f"Model ({self.custom_api_model_var.get()}): ✅ Available"
                            )
                        else:
                            results.append(
                                f"Custom API: ❌ HTTP {response.status_code}"
                            )
                            try:
                ...
                try:
                    ...
                    response = requests.get(
                        f"{self.ollama_url_var.get()}/api/tags", timeout=5
                    )
                    if response.status_code == 200:
                        ...
                        if embedding_model:
                            ...
                            try:
                                ...
                                if embed_response.status_code == 200:
                                    results.append(
                                        f"Embedding Model ({embedding_model}): ✅ Available"
                                    )
                                    embed_success = True
                            except Exception:
                                pass
                            ...
                            if not embed_success:
                                try:
                                    ...
                                    if embed_response.status_code == 200:
                                        results.append(
                                            f"Embedding Model ({embedding_model}): ✅ Available (legacy API)"
                                        )
                                        embed_success = True
                                except Exception:
                                    pass
    ...
    def _save(self):
        ...
        try:
            ...
            try:
                self.config.custom_api.max_tokens = int(
                    self.custom_api_max_tokens_var.get()
                )
            except ValueError:
            ...
            messagebox.showerror(
                "Invalid Configuration", f"Error in configuration:\n{str(e)}"
            )
...
class SessionLoadDialog:
    ...
    def _setup_widgets(self):
        ...
        # Title
        title_label = ttk.Label(
            main_frame, text="Load Session", font=("TkDefaultFont", 12, "bold")
        )
...
class WorkflowDiagram:
    ...
    def set_rag_progress(self, current: int, total: int, active: bool = True):
        ...
        if active and total > 0:
            ...
            self.rag_status_text = (
                f"Saving RAG vectors: {current}/{total} ({percentage}%)"
            )
        else:
            self.rag_status_text = ""
        self._draw_workflow()
...
class MemoryInfoPanel:
    ...
    def _setup_widgets(self):
        ...
        ttk.Label(cag_frame, text="CAG System:", font=("Arial", 10, "bold")).pack(
            side="left"
        )
        ...
        ttk.Label(rag_frame, text="RAG System:", font=("Arial", 10, "bold")).pack(
            side="left"
        )
        ...
        # Memory Stats
        ttk.Label(self.frame, text="Memory Stats:", font=("Arial", 10, "bold")).grid(
            row=2, column=0, sticky="nw", pady=(5, 0)
        )
    ...
    def _update_memory_info(self):
        ...
        try:
            ...
            if hasattr(self.bridge, "config") and hasattr(
                self.bridge.config, "session_history"
            ):
                rag_enabled = getattr(
                    self.bridge.config.session_history, "use_vector_embeddings", False
                )
            ...
            if hasattr(self.bridge, "cag_manager") and self.bridge.cag_manager:
                ...
                if vector_store and rag_enabled:
                    ...
                    if (
                        hasattr(vector_store, "embeddings")
                        and vector_store.embeddings is not None
                    ):
                        try:
                            ...
                            if hasattr(vector_store.embeddings, "shape"):
                                cag_vector_count = (
                                    vector_store.embeddings.shape[0]
                                    if len(vector_store.embeddings.shape) > 0
                                    else 0
                                )
                                vector_count = max(vector_count, cag_vector_count)
    ...
    def _toggle_cag(self):
        ...
        try:
            ...
            if new_state and (
                not hasattr(self.bridge, "cag_manager") or not self.bridge.cag_manager
            ):
                try:
                    from src.cag import CAGManager

                    self.bridge.cag_manager = CAGManager(self.bridge.config)
                    self.bridge.memory_manager = getattr(
                        self.bridge.cag_manager, "memory_manager", None
                    )
            ...
            messagebox.showinfo(
                "CAG System", f"Cache-Augmented Generation system has been {status}."
            )
    ...
    def _get_memory_stats(self) -> str:
        ...
        try:
            ...
            if hasattr(self.bridge, "cag_manager") and self.bridge.cag_manager:
                ...
                stats.append(
                    f"Knowledge Base: {'Enabled' if debug_info.get('enable_kb', False) else 'Disabled'}"
                )
                ...
                if "vector_store" in debug_info:
                    ...
                    if "vector_count" in vs_info:
                        ...
                        stats.append(
                            f"Document Count: {vs_info.get('document_count', 0)}"
                        )
                        stats.append(
                            f"Vector Dimensions: {vs_info.get('dimensions', 'N/A')}"
                        )
                    else:
                ...
                if "session" in debug_info:
                    ...
                    stats.append(
                        f"Decompiled Functions: {s_info.get('decompiled_functions', 0)}"
                    )
                    stats.append(
                        f"Analysis Results: {s_info.get('analysis_results', 0)}"
                    )

                # Add cache statistics
                if (
                    "cache_stats" in debug_info
                    and debug_info["cache_stats"] != "unavailable"
                ):
                    ...
                    stats.append(
                        f"Total Requests: {cache_info.get('total_requests', 0)}"
                    )
            ...
            if hasattr(self.bridge, "analysis_state"):
                ...
                stats.append("\n=== Analysis State ===")
                stats.append(
                    f"Functions Decompiled: {len(state.get('functions_decompiled', set()))}"
                )
                stats.append(
                    f"Functions Renamed: {len(state.get('functions_renamed', {}))}"
                )
                stats.append(f"Comments Added: {len(state.get('comments_added', {}))}")
                stats.append(
                    f"Functions Analyzed: {len(state.get('functions_analyzed', set()))}"
                )
            ...
            if hasattr(self.bridge, "current_goal") and self.bridge.current_goal:
                ...
                stats.append(
                    f"Steps Taken: {getattr(self.bridge, 'goal_steps_taken', 0)}"
                )
                stats.append(f"Max Steps: {getattr(self.bridge, 'max_goal_steps', 0)}")
                stats.append(
                    f"Achieved: {'Yes' if getattr(self.bridge, 'goal_achieved', False) else 'No'}"
                )
...
class RenamedFunctionsPanel:
    ...
    def __init__(self, parent, bridge: Bridge):
        ...
        self.batch_operation_in_progress = (
            False  # Flag to prevent auto-refresh during batch operations
        )
    ...
    def _setup_widgets(self):
        ...
        ttk.Button(
            control_frame, text="Refresh", command=self._update_function_list
        ).pack(side="left", padx=(0, 5))
        ttk.Button(
            control_frame, text="Export", command=self._export_function_list
        ).pack(side="left", padx=(0, 5))
        ttk.Button(
            control_frame, text="Clear All", command=self._clear_all_functions
        ).pack(side="left", padx=(0, 5))

        # Add Load Vectors button
        self.load_vectors_button = ttk.Button(
            control_frame,
            text="Load Vectors",
            command=self._load_vectors_from_functions,
        )
        self.load_vectors_button.pack(side="left", padx=(0, 5))

        # Function count label
        self.count_label = ttk.Label(
            self.frame, text="Functions: 0", font=("Arial", 10, "bold")
        )
        self.count_label.grid(row=1, column=0, columnspan=2, sticky="w", pady=(0, 5))

        # Vector status label
        self.vector_status_label = ttk.Label(
            self.frame, text="Vectors: Not loaded", font=("Arial", 9), foreground="gray"
        )
        self.vector_status_label.grid(
            row=1, column=0, columnspan=2, sticky="e", pady=(0, 5)
        )
        ...
        self.tree = ttk.Treeview(
            self.tree_frame, columns=columns, show="headings", height=8
        )
    ...
    def _load_vectors_from_functions(self):
        ...
        try:
            if hasattr(self.bridge, "analysis_state"):
                renamed_functions = self.bridge.analysis_state.get(
                    "functions_renamed", {}
                )
                function_address_mapping = getattr(
                    self.bridge, "function_address_mapping", {}
                )
                function_count = len(
                    set(
                        list(renamed_functions.keys())
                        + list(function_address_mapping.keys())
                    )
                )
        except:
            function_count = 0
        ...
        if function_count == 0:
            messagebox.showinfo(
                "No Functions", "No analyzed functions found to load into vectors."
            )
            return
        ...
        progress_label = ttk.Label(
            progress_frame,
            text="Loading functions into vector store...",
            font=("Arial", 11, "bold"),
        )
        progress_label.pack(pady=(0, 10))

        progress_status = ttk.Label(
            progress_frame, text="Initializing...", foreground="blue"
        )
        progress_status.pack(pady=(0, 10))

        progress_bar = ttk.Progressbar(
            progress_frame, mode="determinate", maximum=function_count
        )
        progress_bar.pack(fill="x", pady=(0, 10))

        progress_detail = ttk.Label(
        ...
        def load_vectors_worker():
            ...
            try:
                ...
                for item in self.tree.get_children():
                    try:
                        ...
                        if len(values) >= 4:
                            ...
                            ):  # Only process functions with summaries
                ...
                if not functions_to_process and hasattr(
                    self.bridge, "function_address_mapping"
                ):
                            ...
                            functions_to_process.append(
                                {
                                    "address": (
                                        identifier
                                        if self._looks_like_address(identifier)
                                        else "Unknown"
                                    ),
                                    "old_name": (
                                        identifier
                                        if not self._looks_like_address(identifier)
                                        else "Unknown"
                                    ),
                                    "new_name": new_name,
                                    "summary": summary,
                ...
                try:
                    ...
                    logger.info(
                        f"✅ Using {provider} embeddings ({emb_model}) for vector creation"
                    )
                except Exception as e:
                ...
                for batch_num in range(num_batches):
                    ...
                    for func in batch:
                        ...
                        if summary and len(summary) > 0:
                            ...
                            logger.warning(
                                f"Skipping function {func.get('new_name', 'Unknown')} - empty summary"
                            )
                            vectors_failed += 1

                    if not batch_texts:
                        logger.warning(
                            f"Batch {batch_num + 1} has no valid texts to embed"
                        )
                        continue
                    ...
                    for i, (func_data, embedding) in enumerate(
                        ...
                        try:
                            ...
                            logger.warning(
                                f"Failed to add {func_data['new_name']}: {e}"
                            )
                            vectors_failed += 1
                ...
                close_button = ttk.Button(
                    progress_frame, text="Close", command=close_dialog
                )
        ...
        if hasattr(self.bridge, "cag_manager") and self.bridge.cag_manager:
            ...
            if vector_store is None:
                try:
                    ...
                    logger.info(
                        "Created new empty vector store for function embeddings"
                    )
                except Exception as e:
        ...
        with self.dict_lock:
            try:
                ...
                if renamed_functions:
                    logger.info(
                        f"DEBUG: Renamed functions keys: {list(renamed_functions.keys())}"
                    )
                    logger.info(
                        f"DEBUG: Renamed functions values: {list(renamed_functions.values())}"
                    )
                if function_address_mapping:
                    logger.info(
                        f"DEBUG: Address mappings keys: {list(function_address_mapping.keys())}"
                    )
                ...
                if hasattr(self.bridge, "function_address_mapping"):
                    ...
                    logger.warning(
                        "DEBUG: Bridge has no function_address_mapping attribute!"
                    )
                ...
                for identifier in renamed_functions.keys():
                    ...
                    for addr, info in function_address_mapping.items():
                        if (
                            addr == identifier
                            or info.get("old_name") == identifier
                            or info.get("new_name") == identifier
                        ):
                ...
                for address, info in function_address_mapping.items():
                    ...
                    display_address = (
                        address.replace("name_", "")
                        if address.startswith("name_")
                        else address
                    )
                    ...
                    summary = (
                        ...
                        or bridge_summaries.get(old_name, "")
                        or bridge_summaries.get(new_name, "")
                        or self.function_summaries.get(
                ...
                for identifier, new_name in renamed_functions.items():
                    ...
                    summary_key = (
                        f"{address}_{new_name}"
                        if address != "Unknown"
                        else f"{old_name}_{new_name}"
                    )
                    summary = bridge_summaries.get(
                        identifier, ""
                    ) or self.function_summaries.get(
                        summary_key, "No summary available"
                    )
                ...
                if "not found" in str(e).lower():
                    logger.debug(
                        f"Tree item not found during function list update: {e}"
                    )
                else:
    ...
    def _looks_like_address(self, text: str) -> bool:
        ...
        if text.startswith("0x") or (
            len(text) >= 4 and all(c in "0123456789abcdefABCDEF" for c in text)
        ):
            return True
    ...
    def _on_function_double_click(self, event):
        ...
        try:
            ...
            messagebox.showwarning(
                "Warning",
                "The selected function is no longer available. The list may have been refreshed.",
            )
            return
    ...
    def _on_right_click(self, event):
        ...
        context_menu.add_command(
            label="Edit Summary",
            command=lambda: self._edit_summary_by_function_data(
                address, old_name, new_name
            ),
        )
        context_menu.add_separator()
        context_menu.add_command(
            label="Copy Address", command=lambda: self._copy_to_clipboard(address)
        )
        context_menu.add_command(
            label="Copy Name", command=lambda: self._copy_to_clipboard(new_name)
        )
        context_menu.add_separator()
        context_menu.add_command(
    ...
    def _edit_summary_for_item(self, item):
        ...
        self._open_summary_editor(
            address, old_name, new_name, current_summary, item, summary_key
        )
    ...
    def _edit_summary_by_function_data(self, address, old_name, new_name):
        ...
        if hasattr(self.bridge, "function_summaries"):
            ...
            current_summary = self.function_summaries.get(
                summary_key, "No summary available"
            )

        # Create a dedicated window for editing the summary (no tree_item dependency)
        self._open_summary_editor(
            address, old_name, new_name, current_summary, None, summary_key
        )

    def _open_summary_editor(
        self, address, old_name, new_name, current_summary, tree_item, summary_key
    ):
        ...
        char_count_label = ttk.Label(
            summary_frame, textvariable=char_count_var, font=("TkDefaultFont", 8)
        )
            ...
            try:
                ...
                for item in self.tree.get_children():
                    try:
                        ...
                        if (
                            len(values) >= 3
                            and values[0] == address
                            and values[1] == old_name
                            and values[2] == new_name
                        ):
                ...
                try:
                    ...
                    messagebox.showinfo(
                        "Success",
                        "Behavior summary saved to storage (UI will update on next refresh)",
                    )
    ...
    def _remove_function(self, item):
        ...
        if messagebox.askyesno(
            "Confirm", "Remove this function from the renamed list?"
        ):
    ...
    def _remove_function_by_data(self, address, old_name, new_name):
        ...
        if messagebox.askyesno(
            "Confirm", f"Remove function '{new_name}' from the renamed list?"
        ):
    ...
    def _clear_all_functions(self):
        ...
        if messagebox.askyesno(
            "Confirm",
            "Are you sure you want to clear all renamed functions? This action cannot be undone.",
        ):
            try:
        ...
        *,
        ...
        if update_state:
            ...
            with self.dict_lock:
                ...
                logger = logging.getLogger("ollama-ghidra-bridge.ui")
                logger.info(
                    f"Added function to session: {address} | {old_name} -> {new_name}"
                )
                logger.info(
...
class AIResponsePanel:
    ...
    def _setup_widgets(self):
        ...
        self.response_text.grid(
            row=0, column=0, columnspan=3, sticky="nsew", pady=(0, 10)
        )

        # Control buttons
        ttk.Button(self.frame, text="Clear", command=self._clear_responses).grid(
            row=1, column=0, padx=(0, 5)
        )
        # Converted "Save to File" to "Generate Report" per user request
        ttk.Button(
            self.frame, text="Generate Report", command=self._on_generate_report
        ).grid(row=1, column=1, padx=5)
        ttk.Button(self.frame, text="Export JSON", command=self._export_json).grid(
    ...
    def _setup_text_tags(self):
        ...
        if colors:
            ...
            self.response_text.tag_config(
                "header", foreground=colors.info, font=("Consolas", 11, "bold")
            )
            ...
            # Tool/action styling
            self.response_text.tag_config(
                "tool", foreground=colors.warning, font=("Consolas", 11, "italic")
            )
            self.response_text.tag_config(
                "reasoning", foreground="#a0a0a0"
            )  # Subtle gray for reasoning
        else:
            # Fallback colors
            self.response_text.tag_config(
                "header", foreground="#5bc0de", font=("Consolas", 11, "bold")
            )
    ...
    def add_response(
        self, response_type: str, content: str, timestamp: Optional[datetime] = None
    ):
        ...
        formatted_response += (
            f"[{timestamp.strftime('%H:%M:%S')}] {response_type.upper()}\n"
        )
...
class QueryInputPanel:
    ...
    def _setup_widgets(self):
        ...
        # Query input
        ttk.Label(
            self.frame, text="Enter your query:", font=("Arial", 10, "bold")
        ...
        ttk.Label(taskmode_frame, text="Mode:").grid(
        ...
        self.task_mode_combo.bind(
            "<<ComboboxSelected>>", lambda e: self._on_task_mode_change()
        )
        ...
        grep_frame = ttk.LabelFrame(
            grep_container, text="Function Search", padding=(10, 5)
        )
        ...
        self.grep_status = ttk.Label(
            grep_frame, text="Off - No summaries loaded", foreground="gray"
        )
        ...
        self.send_button = ttk.Button(
            button_frame, text="Send Query", command=self._send_query
        )
        self.send_button.pack(side="left", padx=(0, 5))

        ttk.Button(button_frame, text="Clear", command=self._clear_query).pack(
            side="left", padx=(0, 15)
        )

        # Separator
        ttk.Separator(button_frame, orient="vertical").pack(
            side="left", fill="y", padx=(0, 15), pady=2
        )

        # Quick action buttons for common smart tools
        self.analyze_button = ttk.Button(
            button_frame, text="Analyze Function", command=self._analyze_current
        )
        self.analyze_button.pack(side="left", padx=(0, 5))

        self.rename_button = ttk.Button(
            button_frame, text="Rename Function", command=self._rename_current
        )
        ...
        def worker():
            try:
                ...
                monitor_thread = threading.Thread(
                    target=self._monitor_workflow_stage, daemon=True
                )
    ...
    def _on_grep_layer_change(self):
        ...
        try:
            if (
                ...
                if hasattr(self.renamed_functions_panel, "tree"):
                    summary_count = len(
                        self.renamed_functions_panel.tree.get_children()
                    )
        ...
        try:
            ...
            logger.info(
                f"Grep layer {'enabled' if enabled else 'disabled'} ({summary_count} functions available)"
            )
        except Exception as e:
    ...
    def _stop_query(self):
        ...
        if self.query_running:
            ...
            self.response_panel.add_response(
                "User Action", "🛑 Query cancellation requested..."
            )
            self._set_query_running(False)
...
class ToolButtonsPanel:
    ...
    def _setup_widgets(self):
        ...
        progress_frame = ttk.Frame(self.frame)
        progress_frame.grid(
            row=next_row + 1, column=0, columnspan=2, sticky="ew", pady=(5, 0)
        )
        progress_frame.grid_columnconfigure(0, weight=1)
    ...
    def _run_ai_agent_query(self, query: str, tool_name: str):
        ...
        def worker():
            try:
                ...
                # Add query to response panel
                self.response_panel.add_response(
                    f"Smart Tool: {tool_name}", f"Query: {query}"
                )
                ...
                if self.renamed_functions_panel:
                    ...
                    try:
                        ...
                        logger.warning(
                            f"Error refreshing function list after batch operation: {refresh_error}"
                        )
    ...
    def _run_hardcoded_workflow(
        self, tool_name: str, display_name: str, params: dict | None = None
    ):
        ...
        def worker():
            try:
                ...
                if hasattr(self.bridge.ghidra, tool_name):
                    ...
                    try:
                        ...
                        if is_list_tool:
                            ...
                            while True:
                                ...
                                if isinstance(
                                    batch_result, str
                                ) and batch_result.lower().startswith("error:"):
                                    self.response_panel.add_response(
                                        "Error",
                    ...
                    is_error = isinstance(
                        raw_tool_result, str
                    ) and raw_tool_result.lower().startswith("error:")
                    ...
                    if not is_error:
                        ...
                        self.response_panel.add_response(
                            f"Raw Output from {tool_name}", formatted_tool_data
                        )
                        ...
                        # --------------------------------------------------
                        extra_context = ""
                        if tool_name == "list_strings" and isinstance(
                            raw_tool_result, list
                        ):
                            ...
                            for addr in addresses:
                                ...
                                for faddr in fn_addrs:
                                    try:
                                        code = self.bridge.ghidra.decompile_function_by_address(
                                            faddr
                                        )
                                        code_snippet = "\n".join(
                                            code.splitlines()[:60]
                        ...
                        analysis_prompt = self._get_analysis_prompt(
                            tool_name, formatted_tool_data + extra_context
                        )

                        try:
                            ai_analysis = self.bridge.ollama.generate(
                            ...
                            if ai_analysis and ai_analysis.strip():
                                ...
                                self.response_panel.add_response(
                                    "Warning", "AI analysis returned empty response."
                                )
        ...
        def worker():
            try:
                ...
                try:
                    ...
                    ) and current_function_result.lower().startswith("error:"):
                        self.response_panel.add_response(
                            "Error",
                    ...
                    self.response_panel.add_response(
                        "Step 1: Current Function", str(current_function_result)
                    )
                    ...
                    self.response_panel.add_response(
                        "Error", f"Error getting current function: {e}"
                    )
                    return
...
CRITICAL: You MUST include all four sections with the exact headers shown above. Focus on making the suggested name as specific and descriptive as possible."""
                    ...
                    if ai_response and ai_response.strip():
                        self.response_panel.add_response(
                            "Step 2: AI Analysis & Name Suggestion", ai_response
                        )
                        ...
                        for line in lines:
                            ...
                            if "Suggested Name:" in line:
                                ...
                                name_part = (
                                    name_part.replace("**", "").replace("*", "").strip()
                                )
                        ...
                        if not suggested_name:
                            ...
                            camel_case_matches = re.findall(
                                r"\b([a-z][a-zA-Z0-9]*[A-Z][a-zA-Z0-9]*)\b", ai_response
                            )
                            ...
                            for match in camel_case_matches:
                                if (
                                    ...
                                    and not any(
                                        word in match.lower()
                                        for word in ["function", "name", "example"]
                                    )
                                ):
                            ...
                            if not suggested_name:
                                simple_matches = re.findall(
                                    r"\b([a-z][a-zA-Z0-9_]*)\b", ai_response
                                )
                        ...
                        if suggested_name:
                            self.response_panel.add_response(
                                "Step 3a: Extracted Suggested Name", suggested_name
                            )
                            ...
                            try:
                                ...
                                if function_summary and hasattr(
                                    self.bridge, "function_summaries"
                                ):
                                    # Get the function address to use as identifier
                                    current_function_result = (
                                        self.bridge.ghidra.get_current_function()
                                    )
                                    if (
                                        ...
                                        if match:
                                            ...
                                            self.bridge.function_summaries[address] = (
                                                function_summary
                                            )
                                            ...
                                            if (
                                                ...
                                                old_vector_count = len(
                                                    self.bridge.cag_manager.vector_store.documents
                                                )

                                            # Show RAG vector creation status
                                            self.workflow_diagram.set_rag_progress(
                                                0, 1, active=True
                                            )
                                            self.response_panel.add_response(
                            ...
                            if isinstance(
                                rename_result, str
                            ) and rename_result.lower().startswith("error:"):
                                self.response_panel.add_response(
                                    "Error",
                        ...
                        self.response_panel.add_response(
                            "Error", "AI agent returned empty response for analysis"
                        )

                except Exception as e:
                    self.response_panel.add_response(
                        "Error", f"Error during AI analysis step: {e}"
                    )
                    return
    ...
    def _process_single_function_for_bulk_rename(
        self, function_index, full_function_string, enumeration_mode, total_functions
    ):
        """
        ...
        try:
            ...
            if " at " in full_function_string:
                ...
                address = (
                    name_address_match.group(1) if name_address_match else function_name
                )
            ...
            is_generic_name = function_name.startswith(
                ("FUN_", "sub_", "loc_", "unk_", "j_")
            )
            ...
            if enumeration_mode == "rename_only":
                ...
                if (
                    any(
                        function_lower.startswith(prefix)
                        for prefix in accessor_prefixes
                    )
                    and len(function_name) < 15
                ):
                    result["result_type"] = "skipped"
                    result["success"] = True
                    return result
                ...
                if is_generic_name:
                    ...
                    should_process = any(
                        keyword in function_lower for keyword in important_keywords
                    )
            ...
            try:
                function_decompile_result = self.bridge.ghidra.decompile_function(
                    name=function_name
                )
                if isinstance(
                ...
                if (
                    enumeration_mode == "smart_enumeration"
                    and function_decompile_result
                ):
            ...
            try:
                context = self._gather_function_context(
                    function_name, address, max_chars=8000
                )
            except Exception as e:
...
CRITICAL: You MUST include all four sections with the exact headers shown above. Focus on making the suggested name as specific and descriptive as possible."""
                ...
                if ai_response and ai_response.strip():
                    ...
                    if enumeration_mode in ["full_enumeration", "smart_enumeration"]:
                        ...
                        if suggested_name and is_generic_name:
                            ...
                            result["error_msg"] = (
                                "Could not extract function name from AI response"
                            )
                            result["result_type"] = "failed"
                            return result
                    ...
                    try:
                        ...
                        structured_summary = self._parse_ai_response_sections(
                            ai_response
                        )
                    ...
                    # THREAD SAFETY: Acquire lock before writing to shared dictionaries
                    if self.renamed_functions_panel and hasattr(
                        self.renamed_functions_panel, "dict_lock"
                    ):
                        with self.renamed_functions_panel.dict_lock:
                            ...
                            if hasattr(self.bridge, "function_address_mapping"):
                                self.bridge.function_address_mapping[address] = result[
                                    "function_data"
                                ]
                    else:
                    ...
                    if retry_count <= max_retries:
                        logger.warning(
                            f"Empty AI response for {function_name}, retry {retry_count}/{max_retries}"
                        )
                        import time
    ...
    def _create_batch_rag_vectors(self, processed_functions_data):
        ...
        for batch_start in range(0, total_functions, rag_batch_size):
            ...
            for i, func_data in enumerate(batch):
                try:
                    ...
                    self.workflow_diagram.set_rag_progress(
                        current_progress, total_functions, active=True
                    )
                    ...
                    logger.warning(
                        f"Could not add function {func_data['old_name']} to RAG: {e}"
                    )
        ...
        if hasattr(self, "renamed_functions_panel") and hasattr(
            self.renamed_functions_panel, "bridge"
        ):
            try:
    ...
    def _gather_function_context(
        self, function_name: str, address: str, max_chars: int = 8000
    ) -> dict:
        """
                ...
                if isinstance(callers_result, list) and callers_result:
                    ...
                    for c in callers_result[:5]:  # Limit to 5 callers
                        if isinstance(c, dict):
                            ...
                            match = re.search(
                                r"(?:from[:\s]+)?([0-9a-fA-F]{6,})", c, re.IGNORECASE
                            )
                            if match:
                    ...
                    for caller_addr in context["callers"][
                        :3
                    ]:  # Only decompile top 3 callers
                        try:
                            # USE CACHE: This dramatically reduces redundant Ghidra calls
                            caller_code = self._decompile_function_cached(
                                address=str(caller_addr)
                            )
                            if caller_code and not caller_code.lower().startswith(
                                "error"
                            ):
                ...
                if isinstance(callees_result, list) and callees_result:
                    ...
                    for c in callees_result[:5]:  # Limit to 5 callees
                        if isinstance(c, dict):
                            # JSON format: extract from dictionary
                            addr = (
                                c.get("to_address") or c.get("to") or c.get("toAddress")
                            )
                            if addr:
                            ...
                            # Try to extract hex address from string
                            match = re.search(
                                r"(?:to[:\s]+)?([0-9a-fA-F]{6,})", c, re.IGNORECASE
                            )
                            if match:
                    ...
                    for callee_addr in context["callees"][
                        :3
                    ]:  # Only decompile top 3 callees
                        try:
                            # USE CACHE: This dramatically reduces redundant Ghidra calls
                            callee_code = self._decompile_function_cached(
                                address=str(callee_addr)
                            )
                            if callee_code and not callee_code.lower().startswith(
                                "error"
                            ):
        ...
        try:
            ...
            for line in lines:
                ...
                if "Function Analysis:" in line or "**Function Analysis:**" in line:
                    ...
                    sections["suggested_name"] = (
                        name_part.strip().replace("**", "").replace("*", "")
                    )
                    current_section = None
    ...
    def _run_bulk_rename_workflow(
        self, display_name: str, enumeration_mode: str = "rename_only"
    ):
        ...
        def worker():
            ...
            try:
                ...
                processed_functions_data = (
                    []
                )  # Store all processed functions for batch RAG creation
                ...
                try:
                    self.response_panel.add_response(
                        "📋 Step 1", "Retrieving list of all functions (paginated)..."
                    )
                    ...
                    while True:
                        batch_result = self.bridge.ghidra.list_functions(
                            offset=offset, limit=limit
                        )
                        ...
                        if isinstance(batch_result, list):
                            ...
                            raw_batch = [
                                f.strip() for f in batch_result.split("\n") if f.strip()
                            ]
                        else:
                            self.response_panel.add_response(
                                "Error",
                        ...
                        batch = [
                            ...
                            and not f.lower().startswith("error")
                        ]
                        ...
                        if total_count:
                            ...
                            progress_msg = (
                                f"Found {len(all_functions)} functions so far..."
                            )
                        self.response_panel.add_response(
                            "Step 1 Progress", progress_msg
                        )
                    ...
                    valid_functions = [
                        f for f in functions if f and not f.lower().startswith("error")
                    ]

                    if not valid_functions:
                        self.response_panel.add_response(
                            "Warning", "No valid functions found to rename."
                        )
                        return
                    ...
                    enumerated_functions = (
                        0  # Functions analyzed but not renamed (for enumeration)
                    )
                    ...
                    with ThreadPoolExecutor(max_workers=max_workers) as executor:
                        ...
                        for future in as_completed(future_to_function):
                            ...
                            if self.should_stop:
                                self.response_panel.add_response(
                                    "Cancelled", f"🛑 Operation cancelled by user"
                                )
                                ...
                                if processed_functions_data:
                                    ...
                                    rag_count = self._create_batch_rag_vectors(
                                        processed_functions_data
                                    )
                                    self.response_panel.add_response(
                                        "✅ Stop Complete",
                            ...
                            try:
                                ...
                                if completed_count % 10 == 0:
                                    progress_pct = (
                                        completed_count / total_functions
                                    ) * 100
                                    self.response_panel.add_response(
                                        "Progress",
                    ...
                    avg_time_per_function = total_time / max(
                        1, (successful_renames + enumerated_functions)
                    )
                    ...
                    if skipped_functions > 0:
                        ...
                        if enumeration_mode == "smart_enumeration":
                            ...
                            skip_reason_msg += (
                                "• Import/export wrappers (__imp_, __exp_)\n"
                            )
                            ...
                            skip_reason_msg += (
                                "• Non-security-relevant renamed functions\n"
                            )
                        ...
                        self.response_panel.add_response(
                            "⏭️ Skip Summary", skip_reason_msg
                        )
                    ...
                    try:
                        self.response_panel.add_response(
                            "💾 Auto-Save", "Automatically saving session..."
                        )
                        ...
                        analyzed_functions = {}
                        if self.renamed_functions_panel and hasattr(
                            self.renamed_functions_panel, "functions"
                        ):
                            for (
                                addr,
                                func_data,
                            ) in self.renamed_functions_panel.functions.items():
                        ...
                        if (
                            ...
                            if (
                                hasattr(self.bridge.cag_manager, "vector_store")
                                and self.bridge.cag_manager.vector_store
                            ):
                                if hasattr(
                                    self.bridge.cag_manager.vector_store, "documents"
                        ...
                        auto_session_name = (
                            f"BulkRename_{enumeration_mode}_{int(time.time())}"
                        )
                        ...
                        self.session_manager.create_session(
                            ...
                            binary_path=getattr(
                                self.bridge, "current_binary_path", "Unknown"
                            ),
    ...
    def _run_hardcoded_analyze_current_function(self, display_name: str):
        ...
        def worker():
            try:
                ...
                try:
                    decompile_result = self.bridge.ghidra.decompile_function(
                        name=function_name
                    )
                    if isinstance(
                        decompile_result, str
                    ) and decompile_result.lower().startswith("error:"):
                        self.response_panel.add_response(
                            "Error",
                    ...
                    self.response_panel.add_response(
                        "Error", f"Error decompiling function {function_name}: {e}"
                    )
                    return
                ...
                try:
                    ...
                    try:
                        if address:
                            ...
                            xrefs_from = self.bridge.ghidra.get_xrefs_from(
                                address=address
                            )
                            # Ensure lists
                            xrefs_to = (
                                xrefs_to
                                if isinstance(xrefs_to, list)
                                else [str(xrefs_to)]
                            )
                            xrefs_from = (
                                xrefs_from
                                if isinstance(xrefs_from, list)
                                else [str(xrefs_from)]
                            )
                        ...
                        logger.debug(
                            f"Could not fetch xrefs for {function_name}: {_xe}"
                        )
                    ...
                    if ai_analysis and ai_analysis.strip():
                        self.response_panel.add_response(
                            "Step 3: Comprehensive AI Analysis", ai_analysis
                        )

                        # Store the function summary for future reference
                        if hasattr(self.bridge, "function_summaries"):
                            self.bridge.function_summaries[function_name] = (
                                ai_analysis.strip()
                            )
...
Choose your enumeration strategy:"""
        ...
        self._run_bulk_rename_workflow(
            "Rename All Functions", enumeration_mode=selected_mode
        )
...
Do you want to proceed with generating the vulnerability report?"""
        ...
        if not messagebox.askyesno(
            "Generate Vulnerability Report", confirmation_message
        ):
            return
    ...
    def _run_software_report_workflow(self, display_name: str, report_format: str):
        ...
        def worker():
            try:
                ...
                self.response_panel.add_response(
                    "Phase 1", "Collecting comprehensive binary data..."
                )
                self.workflow_diagram.set_current_stage("execution")
                ...
                self.response_panel.add_response(
                    "Phase 3", "Generating structured software report..."
                )
                    ...
                    if save_response:
                        ...
                        if filename:
                            try:
                                ...
                                self.response_panel.add_response(
                                    "File Saved", f"✅ Report saved to: {filename}"
                                )
                            except Exception as e:
                                self.response_panel.add_response(
                                    "Save Error", f"❌ Error saving file: {e}"
                                )
...
"""
    ...
    def _stop_tool(self):
        ...
        if self.tool_running:
            self.should_stop = True
            self.response_panel.add_response(
                "User Action", "🛑 Tool cancellation requested..."
            )
            self._set_tool_running(False)
    ...
    def _search_strings(self):
        ...
        query = sd.askstring(
            "String Search", "Enter substring to search in defined strings:"
        )
        if query is None or query.strip() == "":
            return  # cancelled

        # list_strings supports optional 'filter' parameter (alias string_search)
        self._run_hardcoded_workflow(
            "list_strings",
            f"Search Strings for '{query}'",
        ...
        def worker():
            try:
                ...
                if tables:
                    ...
                    self.response_panel.add_response(
                        f"Scan Complete: Found {len(tables)} Table(s)", formatted
                    )
...
class OGhidraUI:
    ...
    def _setup_menu(self):
        ...
        tools_menu.add_command(
            label="Server Configuration", command=self._configure_servers
        )
        ...
        analysis_menu.add_command(
            label="Analyze Current Function", command=self._menu_analyze_current
        )
        analysis_menu.add_command(
            label="Rename Current Function", command=self._menu_rename_current
        )
        analysis_menu.add_separator()

        # Batch operations
        analysis_menu.add_command(
            label="Rename All Functions", command=self._menu_rename_all
        )
        analysis_menu.add_command(
            label="Generate Software Report", command=self._menu_generate_report
        )
        analysis_menu.add_separator()

        # Analysis tools
        analysis_menu.add_command(
            label="Analyze Imports", command=self._menu_analyze_imports
        )
        analysis_menu.add_command(
            label="Analyze Exports", command=self._menu_analyze_exports
        )
        analysis_menu.add_command(
            label="Analyze Strings", command=self._menu_analyze_strings
        )
        analysis_menu.add_separator()

        # Search/Scan tools
        analysis_menu.add_command(
            label="Search Strings...", command=self._menu_search_strings
        )
        analysis_menu.add_command(
            label="Scan Function Tables", command=self._menu_scan_tables
        )
    ...
    def _save_session(self):
        ...
        try:
            ...
            try:
                ...
                messagebox.showerror(
                    "Import Error", f"Could not import session manager: {e}"
                )
                return
            ...
            info_frame = ttk.LabelFrame(
                main_frame, text="Current Session Info", padding=10
            )
            ...
            ttk.Label(info_frame, text=f"• Analyzed Functions: {functions_count}").pack(
                anchor="w"
            )
            ...
            def save_session():
                try:
                    ...
                    try:
                        ...
                        messagebox.showerror(
                            "Session Creation Error", f"Could not create session: {e}"
                        )
                        return
                    ...
                    try:
                        if (
                            hasattr(self.bridge, "function_summaries")
                            and self.bridge.function_summaries
                        ):
                            for (
                                address,
                                summary,
                        ...
                        if (
                            ...
                            if hasattr(
                                ...
                                for (
                                    ...
                                    self.renamed_functions_panel.function_summaries.items()
                                ):
                    ...
                    try:
                        ...
                        for identifier, info in mapping.items():
                            ...
                            addr = (
                                identifier
                                if self.renamed_functions_panel._looks_like_address(
                                    identifier
                                )
                        ...
                        logger.warning(
                            f"Analyzed functions refinement failed: {refine_err}"
                        )
                    ...
                    try:
                        ...
                        def _looks_like_address(txt: str) -> bool:
                            ...
                            return (
                                len(txt) >= 4
                                and all(c in "0123456789abcdefABCDEF" for c in txt)
            ...
            ttk.Button(button_frame, text="Save Session", command=save_session).pack(
                side="right", padx=(10, 0)
            )
            ttk.Button(button_frame, text="Cancel", command=cancel_save).pack(
                side="right"
            )
    ...
    def _load_session(self):
        ...
        try:
            ...
            list_frame = ttk.LabelFrame(
                main_frame, text="Available Sessions", padding=10
            )
            ...
            for session in sessions:
                try:
                    ...
                    logger.warning(
                        f"Error displaying session {session.get('name', 'Unknown')}: {e}"
                    )
                    ...
                    try:
                        ...
                        logger.error(
                            f"Failed to display session even with fallbacks: {e2}"
                        )
            ...
            def load_selected_session():
                ...
                try:
                    ...
                    session_data = self.session_manager.load_session_streaming(
                        session["id"]
                    )
                    ...
                    if session_data:
                        ...
                        if (
                            ...
                            analyzed_functions = session_data.get(
                                "analyzed_functions", {}
                            )
                            ...
                            processed_ids = (
                                set()
                            )  # canonical_id = address|new_name to avoid duplicates
                            ...
                            for func_data in analyzed_functions.values():
                                ...
                                if (
                                    not addr
                                    or not self.renamed_functions_panel._looks_like_address(
                                        addr
                                    )
                                ):
                                    ...
                                    for cand in (
                                        ...
                                        if (
                                            cand
                                            and self.renamed_functions_panel._looks_like_address(
                                                cand
                                            )
                                        ):
                                            addr = cand
                                ...
                                if not addr:
                                    addr = (
                                        func_data.get("new_name")
                                        or func_data.get("old_name")
                                        or "Unknown"
                                    )

                                canonical_id = (
                                    f"{addr}|{func_data.get('new_name','Unknown')}"
                                )
                                ...
                                if addr not in unique_funcs:
                                    unique_funcs[addr] = {
                                        "address": addr,
                                        "old_name": func_data.get(
                                            "old_name", "Unknown"
                                        ),
                                        "new_name": func_data.get(
                                            "new_name", "Unknown"
                                        ),
                                        "behavior_summary": func_data.get(
                                            "behavior_summary"
                                        )
                                        or func_data.get("summary", ""),
                                    }
                                else:
                                    ...
                                    if not existing.get("behavior_summary"):
                                        existing["behavior_summary"] = func_data.get(
                                            "behavior_summary"
                                        ) or func_data.get("summary", "")

                        for addr, fd in unique_funcs.items():
                            try:
                                summary_val = fd.get("behavior_summary") or fd.get(
                                    "summary", ""
                                )
                                self.renamed_functions_panel.add_function_with_summary(
                                    address=addr,
                                ...
                                logger.warning(
                                    f"Could not restore function {addr}: {e}"
                                )
                        ...
                        success_msg += (
                            f"• Restored {functions_loaded} analyzed functions\n"
                        )
            ...
            ttk.Button(
                button_frame, text="Load Session", command=load_selected_session
            ).pack(side="right", padx=(10, 0))
            ttk.Button(button_frame, text="Cancel", command=cancel_load).pack(
                side="right"
            )
    ...
    def _load_session_streaming(
        self, session_data: Dict[str, Any], session_info: Dict[str, Any]
    ):
        ...
        try:
            ...
            # Title
            title_label = Label(
                content_frame, text=f"Loading Large Session", font=("Arial", 14, "bold")
            )
            title_label.pack(pady=(0, 10))
            ...
            size_label = Label(
                content_frame, text=f"File size: {file_size:.1f} MB", font=("Arial", 10)
            )
            ...
            progress_label = Label(
                content_frame, textvariable=progress_var, font=("Arial", 10)
            )
            progress_label.pack(pady=(10, 5))

            # Progress bar
            progress_bar = ttk.Progressbar(
                content_frame, mode="indeterminate", length=400
            )
            ...
            stats_label = Label(
                content_frame, textvariable=stats_var, font=("Arial", 10, "bold")
            )
                            ...
                            for address, func_data in function_iterator:
                                ...
                                try:
                                    ...
                                    if functions_loaded % 50 == 0:
                                        stats_var.set(
                                            f"Functions loaded: {functions_loaded}"
                                        )
                                        progress_dialog.update_idletasks()

                                except Exception as e:
                                    logger.debug(
                                        f"Could not restore function {address}: {e}"
                                    )
    ...
    def _show_system_info(self):
        ...
        # CAG System Controls
        cag_frame = ttk.LabelFrame(
            main_frame, text="Context-Augmented Generation (CAG)", padding=10
        )
        ...
        cag_check = ttk.Checkbutton(
            cag_frame, text="Enable CAG", variable=cag_var, command=toggle_cag
        )
        cag_check.pack(anchor="w", pady=(5, 0))

        # RAG System Controls
        rag_frame = ttk.LabelFrame(
            main_frame, text="Retrieval-Augmented Generation (RAG)", padding=10
        )
        ...
        def get_vector_count():
            ...
            if hasattr(self.bridge, "cag_manager") and self.bridge.cag_manager:
                ...
                if (
                    vector_store
                    and hasattr(vector_store, "embeddings")
                    and vector_store.embeddings is not None
                ):
                    try:
        ...
        rag_check = ttk.Checkbutton(
            rag_frame, text="Enable RAG", variable=rag_var, command=toggle_rag
        )
        ...
        ttk.Button(rag_frame, text="Refresh Count", command=refresh_rag_status).pack(
            anchor="w", pady=(5, 0)
        )
        ...
        def refresh_stats():
            ...
            try:
                ...
                if hasattr(self.bridge, "function_summaries"):
                    stats.append(
                        f"Analyzed Functions: {len(self.bridge.function_summaries)}"
                    )
                if hasattr(self.bridge, "function_address_mapping"):
                    stats.append(
                        f"Function Mappings: {len(self.bridge.function_address_mapping)}"
                    )
                ...
                if hasattr(self.bridge, "cag_manager") and self.bridge.cag_manager:
                    ...
                    if vs:
                        ...
                        emb_count = (
                            len(vs.embeddings)
                            if hasattr(vs, "embeddings") and vs.embeddings
                            else 0
                        )
                ...
                stats_text.insert(
                    1.0, "Install psutil for detailed memory stats:\npip install psutil"
                )
            except Exception as e:
        ...
        ttk.Button(button_frame, text="Refresh", command=refresh_stats).pack(
            side="left"
        )
        ...
        try:
            ...
            if dialog.result:
                ...
                if hasattr(self.bridge, "reload_llm_client"):
                    ...
                    self.bridge.ollama.base_url = str(
                ...
                if hasattr(self.bridge, "ghidra_client") and self.bridge.ghidra_client:
                    self.bridge.ghidra_client.config.base_url = str(
                        self.config.ghidra.base_url
                    ).rstrip("/")
            ...
            messagebox.showerror(
                "Configuration Error", f"Failed to configure servers: {e}"
            )
