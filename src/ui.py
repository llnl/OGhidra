...
class WorkflowDiagram:
    ...
    def __init__(self, parent, width=400, height=100):
        ...
        self.canvas = tk.Canvas(parent, width=width, height=height, bg=canvas_bg, relief="flat", bd=0, highlightthickness=0)
        self.width = width
        self.height = height
    ...
    def _draw_workflow(self):
        ...
        for i, stage in enumerate(self.stages):
            ...
            self.canvas.create_rectangle(
                x_start, y_center - 15, x_start + stage_width - 10, y_center + 15, fill=color, outline=outline_color, width=1
            )

            # Draw stage text
            self.canvas.create_text(x_center - 5, y_center, text=stage, fill=text_color, font=("Segoe UI", 9, "bold"))

            # Draw arrow to next stage (light color for dark theme)
            if i < len(self.stages) - 1:
                arrow_x = x_start + stage_width - 5
                self.canvas.create_line(arrow_x, y_center, arrow_x + 10, y_center, arrow=tk.LAST, fill="#6a6a6a", width=2)
