...
def is_retryable_exception(e):
    ...
    if isinstance(e, requests.exceptions.HTTPError):
        ...
        return e.response is not None and e.response.status_code in [
            429,
            500,
            502,
            503,
...
class OllamaClient:
    ...
    def __init__(self, config):
        ...
        print(
            f"[OllamaClient] Initialized: timeout={self.timeout}s, request_delay={request_delay}s, max_retries={self.max_retries}, model={self.default_model}"
        )
    ...
    def _embed_chunked(self, text: str, embedding_model: str, start_time: Optional[float]) -> List[float]:
        ...
        self.logger.info(
            f"Successfully embedded {len(chunk_embeddings)}/{len(chunks)} chunks, averaged to single {len(averaged)}-dim vector"
        )
