...
class MemoryInfoPanel:
    ...
    def _update_memory_info(self):
        ...
        try:
            ...
            self._logger.error(f"Error updating memory info: {e}")
            self.memory_text.after(0, lambda: self.memory_text.delete(1.0, tk.END))
            self.memory_text.after(
                0,
                lambda: self.memory_text.insert(1.0, f"Error updating memory info: {e}"),
            )
        ...
        try:
            ...
            if new_state and (not hasattr(self.bridge, "cag_manager") or not self.bridge.cag_manager):
                try:
                    ...
                    self._logger.error(f"Failed to reinitialize CAG Manager: {e}")
            ...
            self._logger.error(f"Error toggling CAG: {e}")
        ...
        try:
            ...
            if new_state:
                ...
                if hasattr(self.bridge, "cag_manager") and self.bridge.cag_manager:
                    ...
                    self._logger.info("Disabled vector store usage for RAG prompts")
            ...
            if new_state and hasattr(self.bridge, "cag_manager") and self.bridge.cag_manager:
                ...
                self._logger.info("Enabled vector store usage for RAG prompts")
            ...
            self._logger.error(f"Error toggling RAG: {e}")
