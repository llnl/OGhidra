                ...
                for batch_num in range(num_batches):
                    ...
                    for func in batch:
                        ...
                        if summary and len(summary) > 0:
                            ...
                            self._logger.warning(f"Skipping function {func.get('new_name', 'Unknown')} - empty summary")
                            vectors_failed += 1

                    if not batch_texts:
                        self._logger.warning(f"Batch {batch_num + 1} has no valid texts to embed")
                        continue
        ...
        if hasattr(self.bridge, "cag_manager") and self.bridge.cag_manager:
            ...
            if vector_store is None:
                try:
                    ...
                    self._logger.info("Created new empty vector store for function embeddings")
                except Exception as e:
        ...
        with self.dict_lock:
            try:
                ...
                if "not found" in str(e).lower():
                    self._logger.debug(f"Tree item not found during function list update: {e}")
                else:
                    self._logger.error(f"Error updating function list: {e}")
    ...
    def _open_summary_editor(self, address, old_name, new_name, current_summary, tree_item, summary_key):
        ...
        def save_and_close():
            ...
            try:
                ...
                self._logger.error(f"Error saving summary: {e}")
                ...
                try:
                    ...
                    self._logger.error(f"Failed to save to storage as well: {inner_e}")
        ...
        def refresh():
            try:
                ...
                if self.batch_operation_in_progress:
                    self._logger.debug("Skipping auto-refresh during batch operation")
                else:
                    self._update_function_list()
            except Exception as e:
