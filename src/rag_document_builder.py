...
class RAGDocumentBuilder:
    ...
    def _format_hierarchical_content(
        self,
        new_name: str,
        address: str,
        old_name: str,
        ...
        sections.append(self._build_quick_reference(categories, security, metrics))
        ...
        # Technical Details
        sections.append(self._build_technical_details(signature, metrics, data_flow))
    ...
    def _build_technical_details(self, signature: Dict, metrics: Dict, data_flow: Dict) -> str:
        ...
        if params:
            param_str = ", ".join(f"{p.get('type', '?')} {p.get('name', '?')}" for p in params[:4])
            if len(params) > 4:
