...
class GhidraMCPClient:
    ...
    def _coerce_int_param(self, value: Any, *, param_name: str, default: int) -> int:
        ...
        if isinstance(value, bool):
            self.logger.warning(f"Invalid {param_name}=bool; using default={default}")
            return default
        ...
        if isinstance(value, str):
            ...
            try:
                ...
                self.logger.warning(f"Invalid {param_name}='{value}'; using default={default}")
                return default
    ...
    def _detect_api(self):
        ...
        try:
            ...
            if (
                ...
                self.logger.info("Successfully connected to GhidraMCP API")
                # Update info for current instance
                if self.current_instance_port:
                    self._update_instance_info(self.current_instance_port)
            else:
                self.logger.warning(f"Failed to connect to GhidraMCP API: {response}")
        except Exception as e:
            self.logger.warning(f"Error detecting API: {str(e)}")
    ...
    def health_check(self) -> bool:
        ...
        try:
            ...
            self.logger.error(f"GhidraMCP server health check failed: {str(e)}")
            return False
    ...
    def check_health(self) -> bool:
        ...
        try:
            ...
            self.logger.debug(f"Querying the Ghidra MCP server for it's status using: {url}")
            response = self.client.get(url, params={"offset": 0, "limit": 1})
            response.raise_for_status()
            return True
        except Exception as e:
            self.logger.error(f"GhidraMCP health check failed: {str(e)}")
            return False
    ...
    def list_imports(self, offset: int = 0, limit: int = 100) -> List[str]:
        ...
        if limit > self.MAX_SAFE_LIMIT:
            self.logger.warning(
                self.LIMIT_WARNING_TEMPLATE.format(method="list_imports", limit=limit, max_safe=self.MAX_SAFE_LIMIT)
            )
            limit = self.MAX_SAFE_LIMIT
    ...
    def list_exports(self, offset: int = 0, limit: int = 100) -> List[str]:
        ...
        if limit > self.MAX_SAFE_LIMIT:
            self.logger.warning(
                self.LIMIT_WARNING_TEMPLATE.format(method="list_exports", limit=limit, max_safe=self.MAX_SAFE_LIMIT)
            )
            limit = self.MAX_SAFE_LIMIT
    ...
    def analyze_function(self, address: str = None) -> str:
        ...
        if address is None:
            ...
            if determined_address is None:
                self.logger.info("analyze_function: Trying get_current_address() as fallback to determine function address.")
                ...
                if (
                    ...
                    self.logger.info(f"analyze_function: Determined address '{determined_address}' from get_current_address().")
                else:
                    self.logger.warning(
        ...
        if self.llm_client:
            try:
                ...
                self.logger.info(f"AI analysis generated for function at {address}")

            except Exception as e:
                self.logger.warning(f"AI analysis failed for function at {address}: {e}. Falling back to raw code.")
        ...
        if function_calls and len(function_calls) > 0:
            ...
            for func_name in list(function_calls)[:3]:
                try:
                    ...
                    self.logger.debug(f"Could not decompile referenced function {func_name}: {e}")
    ...
    def scan_function_pointer_tables(
        ...
        self.logger.info("Building function address lookup table...")
        ...
        if not function_map:
            self.logger.warning("No functions found, cannot scan for tables")
            return []

        # Determine code address range for quick filtering
        min_func_addr = min(function_map.keys())
        max_func_addr = max(function_map.keys())
        self.logger.info(f"Found {len(function_map)} functions in range 0x{min_func_addr:x} - 0x{max_func_addr:x}")

        # Step 2: Get memory segments and identify data segments
        self.logger.info("Analyzing memory segments...")
            ...
            if seg_match:
                try:
                    ...
                    if size > 0:
                        ...
                        self.logger.debug(f"Parsed segment: {seg_name} 0x{start:x} - 0x{end:x} ({size} bytes)")
                except ValueError:
                    continue

        # If we couldn't parse segments, try scanning around function addresses
        if not data_segments:
            self.logger.warning("Could not parse data segments, scanning around function address range")
        ...
        self.logger.info(f"Scanning {len(scannable_segments)} segment(s) for function pointer tables (skipping code segments)")
        ...
        for segment in scannable_segments:
            ...
            self.logger.info(f"Scanning segment {segment['name']}: 0x{segment['start']:x} ({segment['size']} bytes)")
            ...
            if tables_in_segment:
                self.logger.info(f"Found {len(tables_in_segment)} table(s) in segment {segment['name']}")
            results.extend(tables_in_segment)

        # Log summary
        if results:
            self.logger.info(f"Total: Found {len(results)} potential function pointer tables")
        else:
            self.logger.info(f"No function pointer tables found (require {min_table_entries}+ consecutive pointers)")
            self.logger.info("Tip: Some binaries (especially C programs) may not have traditional pointer tables")
    ...
    def _scan_segment_for_tables(
        ...
        for offset in range(0, scan_length, chunk_size):
            ...
            try:
                ...
                self.logger.debug(f"Error scanning at 0x{current_addr:x}: {e}")
                continue
