...
class Bridge:
    ...
    def __init__(
        ...
        self.provider = getattr(bridge_config, "llm_provider", "ollama")

        # Handle 'google' alias for backward compatibility
        if self.provider == "google":
            self.provider = "external"

        # TODO: The these six classes should be refactored so a single type hint can point to a single
        # base class that each external configuration overrides.
        self.llm_config = ExternalConfig | CustomAPIConfig | OllamaConfig
        self.llm_client: ExternalClient | CustomAPIClient | OllamaClient

        if self.provider == "external":
            self.llm_config = bridge_config.external
            self.llm_client = ExternalClient(config=self.llm_config)
            self.logger.info(f"Using External Provider ({self.llm_config.provider}) as LLM")
        elif self.provider == "custom_api":
            self.llm_config = bridge_config.custom_api
            self.llm_client = CustomAPIClient(config=self.llm_config)
            self.logger.info("Using Custom API as LLM provider")
        else:
            self.llm_config = bridge_config.ollama
            self.llm_client = OllamaClient(config=self.llm_config)
        ...
        self.ghidra_client = GhidraMCPClient(config=bridge_config.ghidra, ollama_client=self.llm_client)
        ...
        if self.enable_cag:
            try:
                ...
                self.cag_manager = CAGManager(bridge_config, session=self.session)
        ...
        self.session_compactor = SessionCompactor(self.llm_config, self.llm_client)
    ...
    def reload_llm_client(self):
        ...
        # Select LLM Provider and Config
        self.provider = getattr(self.bridge_config, "llm_provider", "ollama")
        ...
        if self.provider == "external":
            self.llm_config = self.bridge_config.external
            self.llm_client = ExternalClient(config=self.llm_config)
            self.logger.info(f"Switched to External Provider: {self.llm_config.provider}")
        elif self.provider == "custom_api":
            self.llm_config = self.bridge_config.custom_api
            self.llm_client = CustomAPIClient(config=self.llm_config)
            self.logger.info("Switched to Custom API Provider")
        else:
            self.llm_config = self.bridge_config.ollama
            self.llm_client = OllamaClient(config=self.llm_config)
    ...
    def _maybe_update_custom_workplan(self, user_query: str, final_response: str) -> None:
        ...
        try:
            ...
            updated = self.llm_client.generate(
                prompt=prompt,
...
You can help analyze binary files by executing commands through GhidraMCP."""
    ...
    Be constructive and specific in your feedback.
        ...
        structured_prompt = StructuredPrompt(
            ...
            conversation_history=self.session.get_recent_messages(limit=self.bridge_config.context_limit),
            phase_specific_instructions=phase_instructions,
        )

        # Generate user prompt with conversation history ALWAYS at the end
        user_prompt = structured_prompt.build_user_prompt(max_history_items=self.bridge_config.context_limit)
                    ...
                    if primary_addresses:
                        ...
                        for addr in expanded_addresses:
                            if addr not in primary_addresses and addr in self.function_address_mapping:
                                ...
                                results.append(
                                    {"document": doc, "score": 0.3 + (centrality * 0.3)}  # 0.3-0.6 range for graph neighbors
                                )
            ...
            if normalized_command == "list_strings":
                ...
                if category:
                    if category == "filesystem":
                        ...
                        self.logger.info(f"🔄 Converted category='filesystem' to filter='.exe' (approximate)")

                    elif category == "registry":
                        params["filter"] = "HKLM"  # Basic start
                        self.logger.info(f"🔄 Converted category='registry' to filter='HKLM'")

                    elif category == "urls":
                        params["filter"] = "http"
                        self.logger.info(f"🔄 Converted category='urls' to filter='http'")
        ...
        try:
            ...
            for cycle in range(1, max_cycles + 1):
                ...
                if exec_results.pending_question:
                    ...
                    self.logger.info(f"⏸️ Question pending — Phase 1: Log and continue")
    ...
    def _generate_plan(self, query: str) -> str:
        ...
        response = self.llm_client.generate_with_phase(user_prompt, phase="planning", system_prompt=system_prompt)
...
If investigation is incomplete or name is too generic, use EXECUTE to call tools."""
            ...
            review_response = self.llm_client.generate_with_phase(user_prompt, phase="analysis", system_prompt=system_prompt)
    ...
    def _execution_loop(self, plan: str, max_steps: int = 10) -> ExecutionPhaseResults:
        ...
        for step in range(1, max_steps + 1):
            ...
            if reasoning and getattr(self.bridge_config.ollama, "show_reasoning", True):
...
Output ONLY JSON (same structure), top {max_items} items."""
        ...
        try:
            ...
            resp = self.llm_client.generate(prompt=ranking_prompt, system_prompt="Filter. Output JSON only.", phase="execution")
...
"""
        ...
        response = self.llm_client.generate_with_phase(user_prompt, phase="evaluation", system_prompt=system_prompt)
        ...
        if not response or not response.strip():
            ...
            response = self.llm_client.generate_with_phase(retry_prompt, phase="evaluation", system_prompt=system_prompt)
    ...
    def _add_function_to_rag(self, function_identifier: str, func_data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            try:
                ...
                use_multi_vector = (
                    getattr(self.bridge_config, "use_multi_vector_rag", False) if hasattr(self, "config") else False
                )
    ...
    def _perform_review_phase(self) -> List[str]:
        """
        Execute the review phase to analyze the results of the investigation.

        Returns:
            List of result strings from the review
        """
        logging.info("Starting review phase")
    ...
    def _perform_comprehensive_ai_analysis(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            classification_response = self.llm_client.generate(prompt=classification_prompt)
            ...
            security_response = self.llm_client.generate(prompt=security_prompt)
            ...
            function_response = self.llm_client.generate(prompt=function_prompt)
            ...
            behavior_response = self.llm_client.generate(prompt=behavior_prompt)
            ...
            architecture_response = self.llm_client.generate(prompt=architecture_prompt)
            ...
            risk_response = self.llm_client.generate(prompt=risk_prompt)
...
"""
    ...
    def _generate_html_report(self, data: Dict[str, Any], analysis: Dict[str, Any]) -> str:
        ...
        prompt = self.bridge_config.ollama.html_report_generation_prompt
...
"""
    ...
    def _call_llm_for_html_report(self, prompt: str) -> str:
        ...
        try:
            ...
            if hasattr(self, "ollama") and self.llm_client:
...
def main():
    ...
    bridge = Bridge(bridge_config=config, include_capabilities=args.include_capabilities, max_agent_steps=args.max_steps)
