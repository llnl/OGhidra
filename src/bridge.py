...
"""
    ...
    def _emit_gate(self, gate: ExecutionGate):
        ...
        self._emit_cot(
            "Gate",
            f"\u26a0\ufe0f EXECUTION PAUSED: {gate.reason} [trigger={gate.trigger}]",
        )
    ...
    def process_query_with_agentic_loop(self, query: str) -> str:
        ...
        try:
            ...
            for cycle in range(1, max_cycles + 1):
                ...
                self._emit_cot(
                    "Status",
                    f"Execution completed: {exec_results.total_steps} tools executed",
                )
                ...
                if exec_results.gates_triggered:
                    ...
                    self._emit_cot(
                        "Gate",
                        f"⚠️ {gate_count} gate(s) triggered this cycle: {gate_summary}",
                    )
                ...
                if goal_achieved:
                    ...
                    if cycle < max_cycles:
                        ...
                        self._emit_cot(
                            "Status",
                            f"Max cycles ({max_cycles}) reached - returning best effort response",
                        )
