...
class SessionCache:
    ...
    def add_context_item(self, role: str, content: str) -> None:
        ...
        item = ContextItem(role=role, content=content, timestamp=time.time(), token_count=token_count)
    ...
    def add_decompiled_function(self, address: str, name: str, decompiled_code: str) -> None:
        ...
        func = DecompiledFunction(
            address=address, name=name, decompiled_code=decompiled_code, timestamp=time.time(), token_count=token_count
        )
    ...
    def add_renamed_entity(self, old_address_or_name: str, new_name: str, entity_type: str) -> None:
        ...
        entity = RenamedEntity(
            old_address_or_name=old_address_or_name, new_name=new_name, entity_type=entity_type, timestamp=time.time()
        )
    ...
    def add_analysis_result(self, query: str, context: str, result: str) -> None:
        ...
        analysis = AnalysisResult(query=query, context=context, result=result, timestamp=time.time(), token_count=token_count)
    ...
    def prune_cache_for_query(self, query: str, token_limit: int = 4000) -> dict[str, Any]:
        ...
        """
        pruned_cache = {"context_history": [], "decompiled_functions": {}, "renamed_entities": {}, "analysis_results": []}
        ...
        self.logger.info(
            f"Pruned cache for query. Using {total_tokens}/{token_limit} tokens. "
    ...
    def format_entire_cache(self) -> str:
        ...
        for old_name, entity in self.renamed_entities.items():
            output.append(f"* {entity.entity_type}: {old_name} -> {entity.new_name}, Timestamp: {time.ctime(entity.timestamp)}")
        output.append("")
        ...
        try:
            ...
            if os.path.exists(entities_path):
                with open(entities_path, "r", encoding="utf-8") as f:
                    self.renamed_entities = {old: RenamedEntity(**entity) for old, entity in json.load(f).items()}
            ...
            self.logger.info(
                f"Loaded session cache for {self.session_id} with "
