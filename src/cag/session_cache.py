...
class SessionCache:
    ...
    def prune_cache_for_query(self, query: str, token_limit: int = 4000) -> Dict[str, Any]:
        ...
        for address, func in sorted(
            self.decompiled_functions.items(),
    ...
    def save_to_disk(self) -> None:
        ...
        try:
            ...
            with open(functions_path, "w", encoding="utf-8") as f:
                json.dump(
                    {addr: vars(func) for addr, func in self.decompiled_functions.items()},
                    f,
                    indent=2,
                )
            ...
            with open(entities_path, "w", encoding="utf-8") as f:
                json.dump(
                    {old: vars(entity) for old, entity in self.renamed_entities.items()},
                    f,
                    indent=2,
                )
