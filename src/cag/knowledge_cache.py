...
class GhidraKnowledgeCache:
    ...
    def save_to_disk(self) -> None:
        ...
        with open(signatures_path, "w", encoding="utf-8") as f:
            json.dump(
                {"function_signatures": {name: vars(sig) for name, sig in self.function_signatures.items()}},
                f,
                indent=2,
            )
        ...
        with open(patterns_path, "w", encoding="utf-8") as f:
            json.dump(
                {"binary_patterns": {name: vars(pattern) for name, pattern in self.binary_patterns.items()}},
                f,
                indent=2,
            )
        ...
        with open(rules_path, "w", encoding="utf-8") as f:
            json.dump(
                {"analysis_rules": {name: vars(rule) for name, rule in self.analysis_rules.items()}},
                f,
                indent=2,
            )
