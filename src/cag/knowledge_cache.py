...
class GhidraKnowledgeCache:
    ...
    def preload(self, knowledge_files: list[str] = None) -> None:
        ...
        if loaded_files > 0:
            ...
            self.logger.info(
                f"Knowledge cache initialized with {len(self.function_signatures)} function signatures, "
    ...
    def save_to_disk(self) -> None:
        ...
        with open(signatures_path, "w", encoding="utf-8") as f:
            json.dump({"function_signatures": {name: vars(sig) for name, sig in self.function_signatures.items()}}, f, indent=2)

        # Binary patterns
        patterns_path = os.path.join(self.cache_dir, "binary_patterns.json")
        with open(patterns_path, "w", encoding="utf-8") as f:
            json.dump({"binary_patterns": {name: vars(pattern) for name, pattern in self.binary_patterns.items()}}, f, indent=2)

        # Analysis rules
        rules_path = os.path.join(self.cache_dir, "analysis_rules.json")
        with open(rules_path, "w", encoding="utf-8") as f:
            json.dump({"analysis_rules": {name: vars(rule) for name, rule in self.analysis_rules.items()}}, f, indent=2)
